...
def api_excel_export():
    """캠페인 데이터를 .xlsx 파일로 내보내기
    실제 파일 컬럼 구조:
      A(1)=구분, B(2)=WEB, C(3)=시작일, D(4)=종료일,
      E(5)=총예산, F(6)=총예산수식, G(7)=일예산수식,
      H(8)=검색어, I(9)=미션내용, J(10)=정답,
      K(11)=힌트URL, L(12)=업체명, M(13)=일유입목표, N(14)=글자수수식
    병합: A,C,D,E,F,G,M,N → 5행씩
    """
    import openpyxl
    ...
    # 열 너비 (실제 파일 기준)
    col_widths = {1:6, 2:6, 3:12, 4:12, 5:10, 6:10, 7:10,
                  8:14, 9:42, 10:12, 11:36, 12:18, 13:8, 14:8}
    ...
    def thin_border(top_thick=False):
        t_side = Side(border_style="medium" if top_thick else "thin",
                      color="03C75A" if top_thick else "253048")
        s = Side(border_style="thin", color="253048")
        return Border(left=s, right=s, top=t_side, bottom=s)
    def camp_sep_border():
        """캠페인 구분선 (위쪽 진한 선)"""
    ...
    # 병합 열 인덱스 (1-based): A=1, C=3, D=4, E=5, F=6, G=7, M=13, N=14
    MERGE_COLS = {1, 3, 4, 5, 6, 7, 13, 14}

    row_num = 2  # 데이터 시작 행 (1행=헤더)
    for ci, camp in enumerate(campaigns):
        rows_data = camp.get("rows", [])  # 5개 행
        camp_start_row = row_num
        is_first_camp = (ci == 0)

        for ri, rd in enumerate(rows_data):
            # rd[0]=A값, rd[1]=B, ..., rd[13]=N값
            for col_idx in range(14):  # 0~13 → 열 1~14
                col_excel = col_idx + 1
        ...
        # 병합: A,C,D,E,F,G,M,N → camp_start_row ~ camp_start_row+4
...
def _load_workbook_safe(path_or_bytes):
    ...
    if isinstance(path_or_bytes, (str, bytes)) and not isinstance(path_or_bytes, _BytesIO):
...
def api_excel_fill():
    """기존 XLSX 파일 또는 기본 템플릿에 캠페인 데이터를 채워서 반환
    실제 파일 컬럼:
      A(1)=구분, B(2)=WEB, C(3)=시작일, D(4)=종료일,
      E(5)=총예산, F(6)=총예산수식, G(7)=일예산수식,
      H(8)=검색어, I(9)=미션내용, J(10)=정답,
    ...
    start_row    = int(request.form.get("start_row", 2))   # 헤더가 1행이면 데이터는 2행부터
    ...
    # 병합 열 (start_col_idx 기준 오프셋): A=0, C=2, D=3, E=4, F=5, G=6, M=12, N=13
    MERGE_OFFSETS = [0, 2, 3, 4, 5, 6, 12, 13]
    ...
    for ci, camp in enumerate(campaigns):
        ...
        for mr in list(ws.merged_cells.ranges):
            if (mr.min_row <= end_row_pre and mr.max_row >= row_num and
                    mr.min_col >= start_col_idx and mr.max_col <= start_col_idx + 13):
        ...
        for ri, rd in enumerate(rows_data):
            ...
            for col_offset in range(14):
                ...
                # 기존 셀 보존 여부: 빈 값은 덮어쓰지 않음 (수식 열 E,F,G,N 제외)
                skip_cols = {start_col_idx+4, start_col_idx+5, start_col_idx+6, start_col_idx+13}
                if col_excel in skip_cols:
                    # 수식 열: 기존 값 유지
                    pass
                else:
                    if val != "":
                    ...
                    # 업체명(L=col_offset 11), 미션(I=col_offset 8) 정렬
                    if col_offset == 8:  # I: 미션내용
